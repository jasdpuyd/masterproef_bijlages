...
OUTPUT_DIR  = r"C:\Users\Gebruiker\Documents\DATA\Grafieken\Regressie_model1"
...
# Aggregeren: maximum over alle blokken
df["max_G"]  = df[G_cols].max(axis=1)   # maximale gemiddelde geluidswaarde
df["max_M"]  = df[M_cols].max(axis=1)   # maximum maximale geluidswaarde
df["max_PM"] = df[PM_cols].max(axis=1)  # maximaal aantal pieken

# Binaire stance: 0 = negatief, 1 = niet-negatief
df["stance_bin"] = (df["stance_numeriek"] != -1).astype(int)

# Werktabel
data = df[["max_G", "max_M", "max_PM", "stance_bin"]].dropna()
X_raw = data[["max_G", "max_M", "max_PM"]].values
...
predictor_short = ["max_G", "max_M", "max_PM"]

print(f"N respondenten : {len(y)}")
print(f"Negatief (0)   : {(y==0).sum()}")
print(f"Niet-negatief (1): {(y==1).sum()}")
print(f"EPV            : {(y==0).sum() / X_raw.shape[1]:.1f}  (min. aanbevolen: 10)")

# ── 2. Standaardiseren ───────────────────────────────────────────────────────
scaler = StandardScaler()
X_std  = scaler.fit_transform(X_raw)

# ── 3. Logistische regressie via statsmodels (voor p-waarden & OR) ───────────
...
conf    = model_sm.conf_int()          # numpy array, geen DataFrame
OR      = np.exp(coefs)
OR_low  = np.exp(conf[1:, 0])          # array-indexering, geen .iloc
OR_high = np.exp(conf[1:, 1])
n_pred    = len(coefs)
...
vif_data["Predictor"] = ["max_G", "max_M", "max_PM"]  # Model 3: voeg "afstand", "windrichting" toe
vif_data["VIF"] = [variance_inflation_factor(X_std, i) for i in range(X_std.shape[1])]
print(vif_data)
print(f"\nMcFadden pseudo-R² : {mcfadden:.3f}")
print(f"LLR p-waarde       : {llr_pval:.4f}")

# ── 4. sklearn model voor ROC / confusion matrix ─────────────────────────────
...
cv_scores = cross_val_score(clf, X_std, y, cv=5, scoring="accuracy")
print(f"\nCV accuraatheid (5-fold): {cv_scores.mean():.3f} ± {cv_scores.std():.3f}")
...
for i in range(n_pred):
    ...
    ax.text(OR_high[i] + 0.03, y_pos[i],
            f"OR={OR[i]:.2f}{sig}  p={pvals[i]:.3f}",
            va="center", fontsize=12, color=CLR_ACCENT)

ax.axvline(1, color="gray", linestyle="--", linewidth=1.2, zorder=1)
ax.set_yticks(y_pos,)
ax.set_yticklabels(predictor_labels, fontsize=12)
ax.set_xlabel("Odds Ratio (met 95%-BI)", fontsize=15)
ax.tick_params(axis='x', labelsize=12)
ax.set_title("Odds Ratio's — Logistisch regressiemodel 1\n(gestandaardiseerde predictoren)",
             fontsize=20, fontweight="bold", pad=12)
ax.set_xlim(left=max(0, OR_low.min() - 0.2))
note = "* p < 0,05   ** p < 0,01   Referentie: OR = 1 (geen effect)"
fig.text(0.5, -0.02, note, ha="center", fontsize=12, color="gray")
plt.tight_layout()
...
ax.set_title("Verwarringsmatrix — Model 1", fontsize=11, fontweight="bold", pad=10)
plt.tight_layout()
...
# ── Figuur 4: Boxplots predictoren per stancegroep ───────────────────────────
pred_names_long = [
    "Max. gem. geluidsniveau\n(LAeq, dBA)",
    "Max. max. geluidsniveau\n(LAmax, dBA)",
    "Max. aantal\ngeluidspieken"
]
raw_cols  = ["max_G", "max_M", "max_PM"]
fig, axes = plt.subplots(1, 3, figsize=(13, 5))

for i, (col, lbl) in enumerate(zip(raw_cols, pred_names_long)):
    ax = axes[i]
    ...
    ax.set_title(f"{lbl}\n({sig_str})", fontsize=9.5, pad=8)
    ax.set_xticks([1, 2])
    ax.set_xticklabels(["Negatief", "Niet-negatief"], fontsize=9)
    ax.set_ylabel("Waarde", fontsize=9)
...
fig.suptitle("Verdeling predictoren per stancegroep", fontsize=12,
...
# ── Figuur 5: Voorspelde kansen per predictor (marginale effecten) ───────────
fig, axes = plt.subplots(1, 3, figsize=(13, 5))

for i, (col, lbl) in enumerate(zip(raw_cols, predictor_labels)):
    ax = axes[i]
    x_range = np.linspace(data[col].min(), data[col].max(), 200)
    # Houd andere predictoren op hun gemiddelde
    means = data[["max_G", "max_M", "max_PM"]].mean().values
    X_sim = np.tile(means, (200, 1))
    X_sim[:, i] = x_range
    X_sim_std = scaler.transform(X_sim)
    probs = clf.predict_proba(X_sim_std)[:, 1]
    ...
    ax.scatter(data[col], y + jitter,
               ...
               alpha=0.45, s=25, zorder=2)
    ax.plot(x_range, probs, color=CLR_ACCENT, lw=2.2, zorder=3)
    ax.axhline(0.5, color="gray", linestyle="--", linewidth=1, alpha=0.7)
    ax.set_xlabel(lbl, fontsize=9)
    ax.set_ylabel("P(niet-negatief)", fontsize=9)
    ax.set_ylim(-0.08, 1.08)
    ax.set_yticks([0, 0.25, 0.5, 0.75, 1])

fig.suptitle("Marginale voorspelde kansen per predictor\n(andere predictoren op gemiddelde)",
...
# ── Figuur 6: Modelfit-samenvatting ─────────────────────────────────────────
...
for row_i, (label, val) in enumerate(metrics):
    ...
    ax.add_patch(plt.Rectangle((0, row_i/len(metrics)),
                                1, 1/len(metrics),
...
ax.set_title("Modelfitstatistieken — Model 1",
...
print("\n✓ Alle figuren opgeslagen in:", OUTPUT_DIR)