...
OUTPUT_DIR = r"C:\Users\Gebruiker\Documents\DATA\Grafieken\Regressie_model2"
...
# Windrichting → binaire dummy (1 = Gunstig, 0 = Ongunstig [referentie])
...
data = df[["afstand", "windrichting_bin", "stance_bin"]].dropna()
y    = data["stance_bin"].values

# Afstand standaardiseren; windrichting is al binair (niet standaardiseren)
scaler     = StandardScaler()
afstand_std = scaler.fit_transform(data[["afstand"]].values)
X_raw      = np.column_stack([afstand_std, data["windrichting_bin"].values])

predictor_labels = ["Afstand\n(gestandaardiseerd)", "Windrichting\n(Gunstig vs. Ongunstig)"]
raw_cols_plot    = ["afstand", "windrichting_bin"]

print(f"N respondenten    : {len(y)}")
print(f"Negatief (0)      : {(y==0).sum()}")
print(f"Niet-negatief (1) : {(y==1).sum()}")
print(f"EPV               : {(y==0).sum() / X_raw.shape[1]:.1f}")

# ── 2. Statsmodels (p-waarden & OR) ─────────────────────────────────────────
...
# ── 3. Sklearn (ROC, verwarringsmatrix, CV) ──────────────────────────────────
...
ax.set_title("Odds Ratio's — Logistisch regressiemodel 2\n(afstand + windrichting)",
             fontsize=20, fontweight="bold", pad=12)
ax.set_xlim(left=max(0, OR_low.min() - 0.3))
fig.text(0.5, -0.02,
         "* p < 0,05   ** p < 0,01   Referentie windrichting: Ongunstig",
...
ax.set_title("ROC-curve — Model 2", fontsize=11, fontweight="bold")
...
ax.set_title("Verwarringsmatrix — Model 2", fontsize=11, fontweight="bold", pad=10)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "fig3_verwarringsmatrix.png"), dpi=150, bbox_inches="tight")
plt.close()
print("Figuur 3 opgeslagen.")

# ── Figuur 4: Boxplot afstand + staafdiagram windrichting ────────────────────
fig, axes = plt.subplots(1, 2, figsize=(10, 5))

# Afstand: boxplot
ax = axes[0]
grp0 = data[data["stance_bin"] == 0]["afstand"]
grp1 = data[data["stance_bin"] == 1]["afstand"]
...
ax.set_title(f"Afstand tot podium (m)\n({sig_str})", fontsize=10, pad=8)
ax.set_xticks([1, 2])
ax.set_xticklabels(["Negatief", "Niet-negatief"], fontsize=9)
ax.set_ylabel("Afstand (m)", fontsize=9)

# Windrichting: gestapeld staafdiagram
ax = axes[1]
...
ax.set_title(f"Windrichting per stancegroep (%)\n(χ² {sig_str})", fontsize=10, pad=8)
ax.set_xlabel("Windrichting", fontsize=9)
ax.set_ylabel("Percentage (%)", fontsize=9)
ax.set_xticklabels(["Ongunstig", "Gunstig"], rotation=0, fontsize=9)
ax.legend(title="Stance", fontsize=8, title_fontsize=8)
ax.set_ylim(0, 100)
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "fig4_boxplot_staaf.png"), dpi=150, bbox_inches="tight")
plt.close()
print("Figuur 4 opgeslagen.")

# ── Figuur 5: Marginale kansen afstand ──────────────────────────────────────
fig, axes = plt.subplots(1, 2, figsize=(11, 5))

# Afstand: continue curve
ax = axes[0]
x_range     = np.linspace(data["afstand"].min(), data["afstand"].max(), 200)
mean_wind   = data["windrichting_bin"].mean()
afstand_sim = scaler.transform(x_range.reshape(-1, 1))
X_sim       = np.column_stack([afstand_sim, np.full(200, mean_wind)])
probs       = clf.predict_proba(X_sim)[:, 1]
jitter      = np.random.default_rng(42).uniform(-0.01, 0.01, len(data))
ax.scatter(data["afstand"], y + jitter,
...
ax.set_xlabel("Afstand tot podium (m)", fontsize=9)
ax.set_ylabel("P(niet-negatief)", fontsize=9)
ax.set_title("Marginale kans — Afstand\n(windrichting op gemiddelde)", fontsize=9.5)
ax.set_ylim(-0.08, 1.08)
ax.set_yticks([0, 0.25, 0.5, 0.75, 1])

# Windrichting: puntplot met CI
ax = axes[1]
for val, label, clr, xpos in [(0, "Ongunstig", CLR_NEG, 0), (1, "Gunstig", CLR_POS, 1)]:
    mean_afstand_std = scaler.transform([[data["afstand"].mean()]])[0]
    X_pt  = np.array([[mean_afstand_std[0], val]])
    prob  = clf.predict_proba(X_pt)[0, 1]
    ax.scatter(xpos, prob, color=clr, s=120, zorder=3)

ax.set_xticks([0, 1])
ax.set_xticklabels(["Ongunstig", "Gunstig"], fontsize=9)
ax.set_ylabel("P(niet-negatief)", fontsize=9)
ax.set_title("Marginale kans — Windrichting\n(afstand op gemiddelde)", fontsize=9.5)
ax.set_ylim(0, 1)
ax.axhline(0.5, color="gray", linestyle="--", linewidth=1, alpha=0.7)
...
ax.set_title("Modelfitstatistieken — Model 2", fontsize=11, fontweight="bold", pad=12)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "fig6_modelfit.png"), dpi=150, bbox_inches="tight")
plt.close()
print("Figuur 6 opgeslagen.")

print("\n✓ Alle figuren Model 2 opgeslagen in:", OUTPUT_DIR)